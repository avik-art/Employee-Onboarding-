...
function jwt(scopes, subject) {
  const c = creds();
  return new google.auth.JWT({ email: c.client_email, key: c.private_key.replace(/\\n/g, '\n'), scopes, subject: subject || undefined });
}
function sheetsApi() { return google.sheets({ version: 'v4', auth: jwt(['https://www.googleapis.com/auth/spreadsheets']) }); }
function driveApi() { return google.drive({ version: 'v3', auth: jwt(['https://www.googleapis.com/auth/drive']) }); }
...
/* ---------- one-time setup: create tabs + headers ---------- */
